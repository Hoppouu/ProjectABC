...
namespace Network
{
    public class PacketTransmitter : IDisposable
    {
        ...
        public void SendToHost<T>(PacketType packetType, T message) where T : IMessage<T>
        {
            ...
            if (IsHost)
            {
                NetworkPacket packet = ToNetworkPacket(NetworkRole.Client, packetType, message, GetNextSendSequence(NetworkRole.Client));
                _receivedPackets.Enqueue(new ReceivedPacket(packet, _hostEndPoint));
                return;
            }
        ...
        private NetworkPacket ToNetworkPacket<T>(NetworkRole senderType, PacketType packetType, T message, int seq) where T : IMessage<T>
        {
        ...
        {
            NetworkPacket packet = ToNetworkPacket(senderType, packetType, message, seq);
